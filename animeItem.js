...
module.exports = async link => {
  try {
    ...
    const checkForTags = amount => {
      ...
      } else if ($(sideBarInfoStr, html)[6].children[1].children[0].data === 'Synonyms:') {
        amountToBeAdded++
      } else {
        /** default */
        return amount
    ...
    const checkType = amount => {
      if ($(sideBarInfoStr, html)[checkForTags(7)].children.filter(item => item.name === 'a')[0].children[0].data === 'Movie') {
        return amount - 2
      } else return amount
    }
    return {
      name: cleanRegEx($('.spaceit_pad', html)[0].children[2].data),
      type: $(sideBarInfoStr, html)[checkForTags(7)].children.filter(item => item.name === 'a')[0].children[0].data,
      ...
      rankingPopularityMembers: $('.di-ib.ml12.pl20.pt8', html)[0].children.map(item => item.children[1].children.map(item => item.data)),
